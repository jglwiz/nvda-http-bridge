...
3. Choose the smallest operation that answers the request:
   ...
   - Restart: `restart` only after an explicit request, then report the before/after `uptimeMs` values.
...
- Keep the base URL on `http://127.0.0.1`; the client rejects other hosts and never puts the token in a URL.
- Never print, copy, log, or return the token. Let the client read `%APPDATA%\nvda\nvdaHttpBridge.token` internally.
...
- Restart only with the client's external `NVDA+Shift+Q` workflow. Never send a lifecycle gesture through NVDA HTTP Bridge or call an HTTP restart action. Treat a dropped or incomplete health response during shutdown as temporary unavailability and keep polling. The client must verify that health returns with a lower `uptimeMs`; do not report success from key delivery alone.
- Create backups only through the asynchronous NVDA HTTP Bridge backup API by using the client's `backup` workflow. Send the chosen target folder as `targetPath`; normalize it and create a new `nvda` child, including missing target parents. Refuse an existing child, exclude the token file, and preserve the completed backup when deleting or expiring its HTTP job.
...
# Explicitly requested restart using this host's NVDA+Shift+Q shortcut.
python <skill-directory>/scripts/nvda_http_bridge.py restart --nvda-key insert --wait-seconds 30
